...
class SuscriptionAccount{
    ...
    public function suscription($name, $lastName, $cardnumber,$avenueStreet, $buildingHouse, $postalcode, $cvv, $expiredate, $username, $IdMembership){
        ...
        if(empty($this->errorAray)){
            return $this->insertSuscriptionDetails($name, $lastName, $cardnumber, $cvv, $expiredate, $username, $IdMembership, $avenueStreet, $buildingHouse, $postalcode);
        }

        return false;
    }

    private function insertSuscriptionDetails($name, $lastName, $cardnumber, $cvv, $expiredate, $username, $IdMembership, $avenueStreet, $buildingHouse, $postalcode){
        ...
        if($query->execute()) {   // Retorna true si funcionó la inserción en la base de datos, false si no
            ...
            $query4 = $this->connection->prepare("UPDATE User SET UserIsSuscribed = b'1', UserPostalCode = :postalcode, UserAvenueStreet = :avenuestreet,
                                                UserBuildingHouse = :buildinghouse WHERE (Username = :username)"); // Para indicar que está suscrito
